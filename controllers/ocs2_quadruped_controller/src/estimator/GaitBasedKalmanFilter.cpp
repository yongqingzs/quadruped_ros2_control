...
{
    GaitBasedKalmanFilter::GaitBasedKalmanFilter(PinocchioInterface pinocchio_interface,
    ...
    vector_t GaitBasedKalmanFilter::update(const rclcpp::Time& time, const rclcpp::Duration& period)
    {
        ...
        vPino.segment<3>(3) = getEulerAnglesZyxDerivativesFromGlobalAngularVelocity<scalar_t>(
            qPino.segment<3>(3), rbd_state_.segment<3>(info_.generalizedCoordinatesNum));
    ...
    nav_msgs::msg::Odometry GaitBasedKalmanFilter::getOdomMsg()
    {
    ...
    void GaitBasedKalmanFilter::loadSettings(const std::string& task_file, const bool verbose)
    {
        ...
        if (verbose)
        {
            std::cerr << "\n #### Kalman Filter Noise (Gait Based):";
            std::cerr << "\n #### =============================================================================\n";
        }
